<template>
  <v-container align="center">
    <v-card elevation="10" outlined align="center" class="pa-8" width="1200">
      ...
      <v-form ref="form" lazy-validate>
        ...
        <v-row v-for="(item, index) in poll.vote" :key="index">
          ...
          <v-col cols="2" md="1">
            <v-btn icon color="green" class="white--text" @click="addChoice">
              <v-icon>mdi-plus</v-icon>
            </v-btn>
          </v-col>

          <v-col cols="2" md="1">
            <v-btn icon color="red" class="white--text" @click="removeChoice">
...
export default {
  data() {
    return {
      ...
      numberRules: [(v) => !!v || "You must enter number"],
      startDateRules: [(v) => !!v || "You must enter start date"],
      endDateRules: [(v) => !!v || "You must enter end date"],
      choiceRules: [(v) => !!v || "You must enter choice"],
    };
  },
  methods: {
    addChoice(choice) {
